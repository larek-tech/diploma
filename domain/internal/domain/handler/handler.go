...
type sourceController interface {
	CreateSource(ctx context.Context, req *pb.Source, meta *authpb.UserAuthMetadata) (*pb.Source, error)
}
...
func New(sc sourceController, dc domainController, tracer trace.Tracer) *Handler {
	return &Handler{
		sc:     sc,
		dc:     dc,
		tracer: tracer,
